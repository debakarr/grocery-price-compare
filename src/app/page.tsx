...
import type { SearchResponse, MatchedProduct, ProductResult, StoreName } from '@/lib/stores/types'
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {product.mrp && product.mrp > product.price && (
            <PercentBadge mrp={product.mrp} price={product.price} />
          )}
        </div>
      </div>
...
export default function Home() {
  ...
  const doSearch = useCallback(async () => {
    ...
    setData(null)
    try {
      const res = await fetch(`/api/search?q=${encodeURIComponent(query.trim())}&pincode=${pincode}&sort=${sort}`)
  ...
  const groupsByRelevance = useMemo(() => {
    ...
    const sections: { label: string; desc: string; groups: MatchedProduct[] }[] = []
        ...
        <div className="max-w-4xl mx-auto px-4 py-5 sm:py-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-300 hidden sm:block">JioMart Quick · Flipkart Minutes · Spencer&apos;s Jiffy · Vishal Mega Mart</p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-5 sm:py-8">
        ...
        {loading && !data && (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-sm text-slate-400">Searching across all stores...</p>
          </div>
        )}
        ...
        {data && (
          <>
            ...
            {sortedMatches.length === 0 ? (
              ...
              (() => {
                let shown = 0
                const rendered = groupsByRelevance.map(section => {
                  const groups = section.groups.slice(0, Math.max(0, visibleCount - shown))
                  shown += groups.length
                ...
                return (<>{rendered.map(section => (
                  ...
                  <button onClick={() => setVisibleCount(c => c + 10)} className="w-full py-3 text-sm font-medium text-blue-600 hover:text-blue-700 bg-white rounded-xl border border-gray-200 hover:border-blue-200 transition-colors shadow-sm">
                    Show {total - displayed} more products →
                  </button>
                )}</>)
              })()
            ) : (
        ...
        {!data && !loading && !error && (
          <div className="text-center py-20">
            ...
            <p className="text-lg font-medium text-slate-400 mb-1">Compare grocery prices instantly</p>
            <p className="text-sm text-slate-300">Search across JioMart, Flipkart, Spencer&apos;s &amp; Vishal Mega Mart</p>
          </div>
        )}
      </main>

      <footer className="text-center py-4 text-[10px] text-slate-300 border-t border-slate-200/50">
        Grocery Price Compare · Delivery pincode: {pincode}
      </footer>
    </div>
  )
}
